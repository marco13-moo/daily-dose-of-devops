# Platform capability: for low-ops teams

Scale converts an informal convention into an outage mechanism. For **platform capability for low-ops teams**, the decisive question is not whether a team can demonstrate the technology once. It is whether the organisation can operate it repeatedly, audit its decisions, and recover when assumptions fail.

## The operating problem

Treat platform capability as part of a product-oriented internal platform with explicit service ownership and paved-road contracts. Define the consumer, owner, support boundary, change policy, and recovery objective before selecting implementation details. When operating platform capability for low-ops teams, undocumented authority and ambiguous ownership create more risk than a missing feature.
...
- Design for the constraints captured by “for low-ops teams”; document the authority, failure domain, and recovery objective.
